...
import types
#list of all types without any internal references to other python objects;
...
class DeterministicDict(object):
    ...
    def __init__(self, obj):
        ...
        keystrs   = np.array([encode(key) for key in keys])
...
class DeterministicSet(object):
    ...
    __slots__ = ['set','type']
    def __init__(self, obj):
        self.set = sorted(encode(key) for key in obj)
        self.type = type(obj)
...
class DeterministicSerializer(object):
    ...
    basic dilemma; we want to impose a sorting order on collection types to get consistent serialization
    but sorting requires serialization...
    hmm; even solvable in principle?

    find canonical problem; self-referential dict?
    note; dict/set keys or class attrs are always hashable!

    hashable types are marshalled right away
    joblib.hashing basically solves all these problems, no?
    """
    def __init__(self, obj):
        self.memo = {}
        import StringIO
    ...
    def serialize(self, obj):
        ...
        if isinstance(obj, flat_types):
            self.buffer.write(marshal.dumps(obj, -1))
        else:
